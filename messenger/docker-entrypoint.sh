...
# Wait for the minio server to be up.
if [ -n "$MINIO_HOST" ]; then
./wait-for-it.sh "$MINIO_HOST:$MINIO_PORT" -t 0
fi

# Wait for the YugabyteDB TServer to start.
if [ -n "$DB_PRIMARY_ENDPOINT" ]; then
./wait-for-it.sh "$DB_PRIMARY_ENDPOINT:$DB_PORT" -t 0
fi
